...
- [x] Run Ralph sync for each promoted company.
  Verification: `bun run sync:company --company=<slug> --provider=auto --mode=dry-run` succeeded for `abbvie`, `chevron`, `home-depot`, `palantir-technologies`, and `procter-gamble`, validating `content/companies/<slug>/bundle.json` for each company.
  Completion note (2026-03-18): the stricter default Claude CLI args regressed headless syncs, so `config/ralph.providers.example.json` was simplified back to the minimal Claude JSON/debug invocation. `abbvie` then succeeded via `research/runs/abbvie/2026-03-18T09-53-31-446Z/`. A later batch rerun initially hit Claude's `marketplace-schema-invalid` plugin startup failure on `chevron`; temporarily moving `~/.claude/plugins/installed_plugins.json`, `~/.claude/plugins/known_marketplaces.json`, and `~/.claude/plugins/marketplaces/` out of the way for the batch cleared the blocker, and the successful reruns landed under `research/runs/chevron/2026-03-18T11-34-44-204Z/`, `research/runs/home-depot/2026-03-18T11-40-08-399Z/`, `research/runs/palantir-technologies/2026-03-18T11-44-46-812Z/`, and `research/runs/procter-gamble/2026-03-18T11-51-02-893Z/`. The Claude plugin files were restored immediately after the batch.
