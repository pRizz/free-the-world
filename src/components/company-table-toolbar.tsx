...
import { Checkbox, CheckboxControl, CheckboxIndicator, CheckboxInput, CheckboxLabel } from "~/components/ui/checkbox";
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "~/components/ui/collapsible";
import { Select, SelectContent, SelectField, SelectHidden, SelectIcon, SelectItem, SelectLabel, SelectListbox, SelectTrigger, SelectValue } from "~/components/ui/select";
...
export function CompanyTableToolbar(props: ToolbarProps) {
  const sectorOptions: SelectOption[] = [{ value: "all", label: "All sectors" }, ...props.sectors.map(sector => ({ value: sector.id, label: sector.label }))];
  const industryOptions: SelectOption[] = [{ value: "all", label: "All industries" }, ...props.industries.map(industry => ({ value: industry.id, label: industry.label }))];
  const sortMetricOptions: SelectOption<CompanyMetricId>[] = companyTableMetricOptions.map(metric => ({
    value: metric.id,
    label: metric.label,
  }));
    ...
    <FilterPanel>
      <div class="flex flex-col gap-4 lg:flex-row lg:items-end">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <ToolbarSelect label="Sector" value={props.sectorId} options={sectorOptions} onChange={props.onSector} />
          <ToolbarSelect label="Industry" value={props.industryId} options={industryOptions} onChange={props.onIndustry} />
          <ToolbarSelect label="Sort metric" value={props.sortMetricId} options={sortMetricOptions} onChange={props.onSortMetric} />
          <ToolbarSelect label="Direction" value={props.sortDirection} options={sortDirectionOptions} onChange={props.onSortDirection} />
        </div>
      </div>
          ...
          <CollapsibleContent class="mt-4">
            <div class="flex flex-wrap gap-2">
              <For each={companyTableMetricOptions}>
                {metric => (
                  <Checkbox checked={props.visibleMetricIds.includes(metric.id)} onChange={() => props.onToggleMetric(metric.id)}>
