...
export function SelectTrigger(props: ParentProps<ComponentProps<typeof KSelect.Trigger> & { class?: string }>) {
...
export function SelectValue<T>(props: ParentProps<any>) {
  const [local, rest] = splitProps(props, ["class"]);
...
export function SelectContent(props: ParentProps<any>) {
  const [local, rest] = splitProps(props, ["class", "children"]);

  return (
    <KSelect.Portal>
      <KSelect.Content
        class={cn("z-50 overflow-hidden rounded-2xl border border-border bg-card p-1 text-foreground shadow-2xl shadow-black/30", local.class)}
        {...rest}
      >
        {local.children}
      </KSelect.Content>
...
export function SelectListbox(props: ParentProps<any>) {
  const [local, rest] = splitProps(props, ["class", "children"]);
  return (
...
export function SelectItem<T>(props: ParentProps<any>) {
  ...
  return (
    ...
    >
      ...
      <KSelect.ItemIndicator class={cn("absolute right-3 inline-flex items-center", (local as any).indicatorClass)}>
        <Check class="h-4 w-4" />
...
export function SelectField(props: ParentProps<{ class?: string }>) {
