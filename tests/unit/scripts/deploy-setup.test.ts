...
test("parseGitHubRepositorySlug supports ssh, https, and bare owner/repo formats", () => {
  ...
  expect(parseGitHubRepositorySlug("https://github.com/pRizz/free-the-world.git")).toBe("pRizz/free-the-world");
...
test("buildGithubOidcTrustPolicy scopes the role to the production environment subject", () => {
  ...
  expect(policy.Statement[0]?.Condition.StringEquals["token.actions.githubusercontent.com:sub"]).toBe(
    "repo:pRizz/free-the-world:environment:production"
  );
});
...
test("buildAwsDeployPolicy scopes Route 53 and S3 resources to the deployment account", () => {
  ...
  const route53Statement = policy.Statement.find(statement => statement.Sid === "Route53RecordChanges");
